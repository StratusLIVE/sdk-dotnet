...
namespace AuthorizeNet.CIM
{
    public class CreateCustomerProfilePaymentFromTransaction
    {
        ...
        private static readonly ILogger Logger = LogFactory.getLog(typeof(CreateCustomerProfilePaymentFromTransaction));
        public ANetApiResponse CreateProfileFromTransaction(Environment env, createCustomerProfileFromTransactionRequest request)
        {
            ...
            //this log is for debugging only. sensitive info.
            ...
            Logger.LogDebug(string.Format("Retreiving Response from Url: '{0}'", postUrl));
            ...
            {
                Logger.LogDebug(string.Format("Received Response: '{0}'", webResponse));
                ...
                using (var responseStream = webResponse.GetResponseStream())
                {
                    if (null != responseStream)
                    {
                        ...
                        Logger.LogDebug(string.Format("Response from Stream: '{0}'", responseAsString));
                    }
                }
            }
        ...
        private static Uri GetPostUrl(Environment env)
        {
            ...
            Logger.LogDebug(string.Format("Creating PostRequest Url: '{0}'", postUrl));
